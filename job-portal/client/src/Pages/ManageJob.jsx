...
const ManageJob = () => {
  ...
  // function to fetch company job apply
    ...
    const changeJobVisibility = async (id) => {
      try {
        const {data} = await axios.post(backendUrl+'/api/compaany/change-visibility',
          {id},
          {headers: {token: companyToken}}
        )
  ...
  return  jobs ? jobs.length === 0 ? (
   ...
   <p className='tex-xl sm:text-2xl'>No Jobs Available or posted</p>
  </div>
  ) : (
